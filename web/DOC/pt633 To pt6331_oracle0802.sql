...
--移除历史遗留数据
DELETE FROM SYS_MENU_DIRECTORY t1 WHERE t1.ID='8a58cd605c1b85cc015c1ba2fab50091';
DELETE FROM SYS_RESOURCE t1 WHERE t1.ID='8a58cd605c1b85cc015c1ba2fab50091';
DELETE FROM SYS_ACCESSCONTROL t1 WHERE t1.ID='402882135d3b0efb015d3b1799cb022a';
...
comment on column SYS_USER.CONSOLE_TYPE is '前后台类型标识 0:后台用户 1:前台用户';

create table SYS_USER_SECRET
(
  id               VARCHAR2(50) not null,
  user_id          VARCHAR2(50) not null,
  org_id           VARCHAR2(50) not null,
  secret_level     VARCHAR2(50) not null,
...
;
comment on table SYS_USER_SECRET
  is '系统用户多组织密级表';
comment on column SYS_USER_SECRET.id
  is '主键ID';
comment on column SYS_USER_SECRET.user_id
  is '用户ID';
comment on column SYS_USER_SECRET.org_id
  is '多组织ID';
comment on column SYS_USER_SECRET.secret_level
  is '用户密级';
comment on column SYS_USER_SECRET.last_update_date
  is '最后修改时间';
comment on column SYS_USER_SECRET.last_updated_by
  is '最后修改人';
comment on column SYS_USER_SECRET.creation_date
  is '创建时间';
comment on column SYS_USER_SECRET.created_by
  is '创建人';
comment on column SYS_USER_SECRET.last_update_ip
  is '修改IP';
comment on column SYS_USER_SECRET.version
  is '版本';
alter table SYS_USER_SECRET
  add primary key (ID);
...
update eform_custom_button t set t.tab_form_id = (select a.id from eform_tab_form a where a.eform_form_info_id = t.form_id and a.attribute_08 = '1') where t.form_id = (select a.eform_form_info_id from eform_tab_form a where a.eform_form_info_id = t.form_id and a.attribute_08 = '1');

--更新控件属性表的唯一标识字段为TAB_FORM_ID，其值为eform_tab_form的id
update EFORM_TAB_COLUMNS_DETAIL t set t.TAB_FORM_ID = (select s.id from EFORM_TAB_FORM s where s.EFORM_FORM_INFO_ID = t.FORM_INFO_ID) where t.form_info_id = (select d.eform_form_info_id from EFORM_TAB_FORM d where d.attribute_08 = '1' and d.eform_form_info_id = t.form_info_id );

--更新主子关系标的目标字段为eform_tab_form的id
update EFORM_TABLE_RELATION t set t.TARGET_ID = (select s."ID" from EFORM_TAB_FORM s where s.EFORM_FORM_INFO_ID = t.TARGET_ID and a.attribute_08 = '1') where t.TARGET_ID = (select a.eform_form_info_id from EFORM_TAB_FORM a where a.attribute_08 = '1' and a.eform_form_info_id=t.TARGET_ID);

update eform_form_info t set t.current_version = 'V1' where t.current_version is null;

update eform_tab_form t set t.attribute_08 = 'V1' where t.eform_form_info_id in (select a.id from eform_form_info a) and t.attribute_08 = '1';
